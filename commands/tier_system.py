...
from config import (
    ...
    SUPABASE_URL, ELO_TICKET_CATEGORY_ID, LEGACY_TICKET_CATEGORY_ID
)
from commands.ban_enforcement import is_banned_by_role
from config import get_gamemode_display_name, POINTS
from database import get_linked_minecraft_name_async, supabase_select, get_session, supabase_headers, supabase_update, supabase_insert

# ==========================================
# ADATTÁROLÓK (JSON fallback)
# ==========================================
...
def get_ticket_category(guild: discord.Guild, mode_key: str) -> Optional[discord.CategoryChannel]:
    """Visszaadja a fix ticket kategóriát: Modern -> ELO_TICKET_CATEGORY_ID, Legacy -> LEGACY_TICKET_CATEGORY_ID."""
...
# ==========================================
# TESZT SZATYOR GOMBOK (VÁRÓLISTA UTÁN)
# ==========================================
...
class QueueActiveView(discord.ui.View):
    ...
    async def toggle_player(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if self.tester_role in interaction.user.roles:
            if interaction.user.id in q_data["testers"]:
                q_data["testers"].remove(interaction.user.id)
                await interaction.followup.send("Leléptél a teszter listáról.", ephemeral=True)
            else:
                q_data["testers"].append(interaction.user.id)
                await interaction.followup.send("Beléptél a teszter listára.", ephemeral=True)
        else:
    ...
    async def close_queue(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.followup.send("✅ Várólista sikeresen lezárva.", ephemeral=True)
...
class HTRequestButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if not category:
            ch_name_to_search = f"⏳-{self.mode_label.lower().replace(' ', '-')}"
            target_ch = discord.utils.get(interaction.guild.text_channels, name=ch_name_to_search)
            category = target_ch.category if target_ch else None
...
class OpenQueueButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        target_ch = discord.utils.get(interaction.guild.text_channels, name=ch_name)
        if not target_ch:
            return await interaction.followup.send(f"❌ Nem találom a(z) `{ch_name}` csatornát!", ephemeral=True)
        ...
        msg = await target_ch.send(content=f"🔔 {q_ping}", embed=embed, view=QueueActiveView(self.mode_key, tester_role))
        ACTIVE_QUEUES[target_ch.id]["msg_id"] = msg.id
        await interaction.followup.send(f"✅ Várólista megnyitva itt: {target_ch.mention}", ephemeral=True)
    ...
    async def setup_cmd(self, interaction: discord.Interaction, modern_kategoria: discord.CategoryChannel, legacy_kategoria: discord.CategoryChannel):
        ...
        try:
            for label, key, _ in ALL_TICKET_TYPES:
                is_legacy = any(k == key for _, k, _ in LEGACY_TICKET_TYPES)
                target_cat = legacy_kategoria if is_legacy else modern_kategoria
                ...
                ch_name = f"⏳-{label.lower().replace(' ', '-')}"
                if not discord.utils.get(target_cat.text_channels, name=ch_name):
                    await guild.create_text_channel(name=ch_name, category=target_cat)
                
                await asyncio.sleep(1)

            await interaction.followup.send("✅ Rendszer sikeresen telepítve: Színezett Rangok és Csatornák létrehozva!")
        except Exception as e:
            await interaction.followup.send(f"❌ Hiba történt a setup közben: `{e}`")

    @app_commands.command(name="revertsetup", description="Törli az összes setup által létrehozott rangot és csatornát.")
    async def revertsetup_cmd(self, interaction: discord.Interaction):
        ...
        try:
            for label, key, _ in ALL_TICKET_TYPES:
                tester_name = f"{label} Teszter"
                queue_name = f"{label} Queue"
                ch_name_new = f"⏳-{label.lower().replace(' ', '-')}"
                ch_name_old = f"queue-{label.lower().replace(' ', '-')}"
                ...
                for channel in guild.text_channels:
                    if channel.name in [ch_name_new, ch_name_old]:
                        await channel.delete()
                
                await asyncio.sleep(1)

            await interaction.followup.send("✅ Setup sikeresen visszavonva: Minden generált rang és csatorna (régi és új) törölve!")
        except Exception as e:
